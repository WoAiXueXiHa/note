...
```
(https://i-blog.csdnimg.cn/direct/c2e3addde5874584ad7af1f689362f76.png)
...
C语言不支持函数重载，主要是因为它的编译器在链接阶段使用**函数名本身**来标识一个函数，函数名在C语言中必须是唯一的；而C++支持函数重载，是因为：

**C语言编译器在编译时，函数名在符号表中就是原名，如(`add`)，所以会发生命名冲突**

**C++编译器会对函数名进行修饰，将参数类型编码进函数名中**
...
```
(https://i-blog.csdnimg.cn/direct/624f11c3b2814f0abd57a4c0eca6801f.png)

这里`a` `na` `nb` `nc` 是同一个值
而改变`na`的值会发生什么？
(https://i-blog.csdnimg.cn/direct/448992691d34438ea3ab0135b44527a8.png)
